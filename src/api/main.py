...
class FinancialExtractor:
    def __init__(self):
        ...
        self.statement_headers = {
            "balance_sheet": ["balance sheet",
                               ...
                               "financial position"],
            "profit_loss"  : ["profit and loss", "income statement",
    ...
    def _parse_number(self, text: str):
        ...
        if text in ["","-","—","nil","Nil","NIL","n/a","N/A"]:
            return None
        negative = ("(" in text and ")" in text) or \
                    text.startswith("-")
    ...
    def from_pdf(self, path: str):
        ...
        for i, line in enumerate(lines):
            ...
            for item, keywords in \
                    self.line_items.get(curr_stmt, {}).items():
                ...
                for kw in keywords:
                    if kw in line.lower():
                        ...
                        nums = re.findall(
                            r"\(?\d[\d,]*(?:\.\d+)?\)?", search)
                        parsed = [abs(self._parse_number(n))
                                  for n in nums
                                  if self._parse_number(n)]
                        if parsed:
                            self.statements[curr_stmt][item]\
                                = parsed[0]
                        break
        self._apply_scale()
...
# PREDICTION HELPER
# ══════════════════════════════════════════════════════════
def run_prediction(statements: dict, years: int,
                   company_name: str, app_state):
    ...
    r["No-credit Interval"]              = sd(ca-inv,
                                              (rev/365)+eps)
    ...
    xgb_p   = float(
        app_state.xgb.predict_proba(X_scaled)[0][1])
    stack_p = float(
        app_state.stack.predict_proba(X_scaled)[0][1])
    ...
    risk_cat = ("HIGH"   if max_p > 60 else
                "MEDIUM" if max_p > 25 else "LOW")

    items_found = sum(
        len(v) for v in statements.values())
    ...
    return {
        ...
        "yearly_probabilities" : {
            int(k): float(v) for k, v in yearly.items()},
...
async def predict_upload(
    ...
    try:
        ...
        if items < 5:
            raise HTTPException(
                422,
                f"Could not extract enough data. "
                f"Only {items} items found."
            )
...
async def predict_manual(data: dict):
    ...
    try:
        ...
        statements = {
            ...
            "profit_loss": {
                k: v for k,v in fin.items()
                if k in extractor.line_items["profit_loss"]
            },
            "cash_flow": {
                k: v for k,v in fin.items()
                if k in extractor.line_items["cash_flow"]
            },
        }
...
async def debug_text(file: UploadFile = File(...)):
    ...
    try:
        matches = []
        keywords = ["revenue", "turnover", "net sales",
